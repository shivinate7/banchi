...
# THE APP'S BUILD STATE, WRITTEN HERE AND READ BY `make status` (D132). One file, the shape
...
# WHAT A `vite build` READS, AND IT IS A SEPARATE SET ON PURPOSE (D132). A change here does
...
# THE APP IS NO LONGER A CHILD (D132) AND THIS ENTRY OUTLIVES IT ON PURPOSE. A supervisor
...
def report(root: Path = REPO_ROOT) -> dict:
    ...
    return {
        ...
        # second pid and a second port (D132). `dev_port` stays because `make dev` still uses
...
def print_where(root: Path = REPO_ROOT) -> None:
    """ONE LINK, because there is one server (D132). The app and the API are the same origin
...
class Supervisor:
    ...
    def start(self) -> None:
        """One child, and the app built into the directory it serves from (D132).
    ...
    def _check_app(self) -> None:
        """The app's own watch: a change here builds, and restarts nothing (D132).
...
def do_restart(args: argparse.Namespace) -> int:
    """`make up ARGS=--restart` is the spelling now (D132 §1.3). Kept for one release.
...
def do_guard_foreground(_args: argparse.Namespace) -> int:
    ...
    THE NARROWING IS D132's, AND IT IS THE POINT RATHER THAN A RELAXATION. This guard existed
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # `--restart` folds `restart` into the verb a person already types (D132 §1.3). The
