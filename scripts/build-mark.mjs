...
/* THE TAB'S PAINT COMES OUT OF SECTION 18, not out of a literal here and not out of section 9's
   locked palettes. It is flat gold — the one paint that reads on a dark browser tab bar AND a
   light one, which a groundless icon needs and neither the mark's silver nor its ink can do.
   NOT ADDED TO markPalettes.ts, deliberately: those six are an illustration's palette (D102) and
   this is one surface's paint. `make docs-audit`'s `rail mark` row reconciles this read against
   section 18's own table, so the spec stays the state of record for it. */
const specMd = readFileSync(resolve(ROOT, 'docs/specs/logo.md'), 'utf8')
const tabPaint = /\| paint \| \*\*flat `(#[0-9A-Fa-f]{6})`\*\*/.exec(specMd)?.[1]
...
must(tabPaint, 'logo.md section 18 has no `| paint | **flat `#RRGGBB`** |` row — the tab\'s colour '
     + 'is settled there and read from there, never held here')
...
function tabSvg(px) {
  const one = `<path d="${round3(tab.body)}" fill="${tabPaint}"/>`
    + tab.caps.map((c) => `<circle cx="${round(c[0])}" cy="${round(c[1])}" r="${round(c[2])}" fill="${tabPaint}"/>`).join('')
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="${px}" height="${px}">
  <title>Banchi</title>
  <!-- GENERATED by scripts/build-mark.mjs. docs/specs/logo.md is the state of record.
       THE EMPTY SLOT (section 1), settled for the tab in section 18: the mark's own L at the
       small cut's ${SMALL.stroke} stroke, carrying the lockup's ${TAB.tip} taper, in flat
       ${tabPaint}. No tile, no card, no sheen, and no gradient: with no ground of its own the tab
       must read on a dark browser bar AND a light one, and gold is the only paint here that does.
       Section 18 has the measurements. The app icons keep the whole mark and its metal. -->
  ${one}
...
console.log(`  the tab: the empty slot, stroke ${SMALL.stroke}, taper ${TAB.tip}/${TAB.tl}, flat ${tabPaint}, no tile`)
