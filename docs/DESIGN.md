...
- **Same-role buttons stacked in one sector share a width** (`D-equal-width-action-stacks`):
...
**Every screen shares one left edge, and only its width may vary** (`D-one-left-edge`).
...
screen` row (D-no-mechanism-on-screen) refuses a decision citation, a repository path, or a
...
(D-copy-ratchet), run by `make design-check`, against the ceilings pinned in
