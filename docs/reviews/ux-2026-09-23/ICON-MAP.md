# Iconography map for Banchi wave 2

This map covers each press in the current code. I found about 340 `<Button>` and 150 raw `<button>` sites. Each row is one line.

Key to the columns:
- **Add** means the kit lane must add the icon to `ICON`.
- **yes** means the icon is already in `ICON`.
- **–** means the press keeps its words.
- A glyph arrow (for example `x → trash`) means the press keeps its words, but its glyph changes to fix a collision (section 3).

## 1. Tables per lane

### Kit (the `ux/kit-icons` lane converts these, because they are kit files)
...
| Refresh (after the load) | Revenue.tsx:Revenue | ICON via ReloadButton hotkey={false} | refresh | yes |
...
| Sort headers | Revenue.tsx:Revenue | WORDS via the kit SortHeader | – | – |
...
| Press | Component | Decision | Icon | In ICON? |
|---|---|---|---|---|
| Reload | Graveyard.tsx:Graveyard | ICON via ReloadButton | refresh | yes |
...
| Reload | Codes.tsx:Codes | ICON via ReloadButton | refresh | yes |
...
| Mark sold (x per pick) | Orders.tsx:PickLine | ICON, not primary tone. Glyph `hand → sold` (risk 4.1) | sold | Add |
...
### Review (the Runs files are here because Runs folds into Review)
| Press | Component | Decision | Icon | In ICON? |
|---|---|---|---|---|
...
| Reload the queue (header) | ReviewQueue.tsx:ReviewQueue | ICON via ReloadButton | refresh | yes |
...
| Reload boxes and runs | Runs.tsx:Runs | ICON via ReloadButton | refresh | yes |
...
| Reload | Pricing.tsx:Pricing | ICON via ReloadButton | refresh | yes |
...
| Keep open | PriceHistory.tsx | WORDS. Drop `pin` (pin means a pick location) | – | – |
...
| Show every card (x in the search field, 44px) | Fulfillment.tsx | Keep as it is: the field-clear pattern, and it meets the 44px floor | – | – |
...
1. **Fulfiller route** (`persona: 'fulfiller'`, which is `Fulfillment.tsx`, `PullConfirm.tsx` and `CardLocations:FulfillerCard`): WORDS always.
   - DESIGN.md sets these floors: text 20px or more, targets 44px or more (IconButton is 40), gaps 12px or more, contrast 7:1, a banned-word list, and an undo on every mark-sold.
   - A touch screen shows no tooltip. The Fulfiller is retired and not technical, so the words must teach.
2. **Money, TCGplayer or no way back.** The press spends money, writes to TCGplayer, or no undo can reverse it: WORDS. Examples are Send, Identify, Spend, Stand down, Reserve, Release a claim, Rebind, Take back and Delete permanently. `danger-solid` is always WORDS.
3. **A fact on the face.** The face carries a count, a price, a name, a box, or a current value or state (a filter value, a sort key): WORDS.
4. **The only primary in its sector.** Examples are a page primary, an empty-state door, a notice's recovery ("Try again"), a sheet or wizard footer, and a decision pair (Cancel, Not now, Back): WORDS.
5. **Other word-only controls.** A menu item, a tab, a segmented option, a chip, a link or press inside a sentence, or a skip link: WORDS.
6. **The verb is in the vocabulary**, and no step above matches: ICON through `IconButton`.
   ...
   - Search → search
   - Drag → grip
7. **Anything else**: WORDS. To make a new icon press, add a path and an `ICON_MEANINGS` row first. A new meaning gets a new path.

Edge cases:
- **The only primary in its sector.** The same verb is ICON in a row and WORDS when it is the only primary. For example, Inventory's row "Mark sold" is ICON, and the phone bar "Mark sold" is WORDS. A list of per-row primaries is not "the only primary", so each row's press is ICON and does not use the primary tone.
- **Meaning changes by state.** Use one footprint (D118).
  - A toggle of one act (Hold/Release, Reveal/Hide, Collapse/Expand) uses `pressed`. Change the label and the icon together, never the icon alone.
  - When the act itself changes (Mark sold, then Undo, or Value my stock, then Refresh), decide each state by the rule. Reserve the wider slot.
  - Copy: the name stays "Copy …", the glyph can show check for 2s, and a polite status says "Copied".
- **Dense row versus alone.**
  - A press repeated per item (row, table, walk, list) is ICON. The same verb alone (empty state, notice, toast, sheet footer) is WORDS.
  - A row holds 3 IconButtons at most. Put the 4th and later presses in a `more` menu with words.
  - In a row, the accessible name carries the object ("Undo the sale at Section 2, Card 5"). The tooltip stays short.
  - A receipt's Undo is ICON, because the receipt sentence teaches. A toast action is WORDS, because the kit label is generic and the toast is short-lived.
- **Phone widths (below 640).** No hover, so no tooltip.
  - The kit shows the tooltip on hover, on keyboard focus and on a long press.
  - A Banchi-specific glyph (sold, archive, moveTo, eraser, lock) goes only where a sentence, pill or heading near it names the act. If nothing names it, the press goes into the `more` menu with words.
  - The hit area is 40px at every width (UX-267).
- **Destructive but undoable.** It never uses `trash` and never uses danger tone. Clear typed uses eraser. Retire uses archive, opens its reason panel and follows UX-252.
- **Destructive and not undoable.** The trash ICON only opens a ConfirmSheet, and the confirm press is WORDS. A cheap loss with a way to redo it (Forget an in-memory file) can be a trash ICON with danger tone that acts at once.
- **Keycap.** It goes in the tooltip, after the label ("Undo U"), never on the face (UX-145, UX-040).

**The kit check** (`icon-only-button` in `scripts/kit-adoption.mjs`). Outside `app/src/kit/**`, the Fulfiller files and Gallery, it refuses three things:
- (a) `iconOnly` on `Button`.
- (b) A raw `<button>` or inline `<svg>` whose only child is an icon.
- (c) A `Button` whose literal label starts with a vocabulary verb (Mark sold, Undo, Retire, Edit, Rename, Delete, Forget, Copy, Download, Open as page, Close, Dismiss, Reload, Manage, Options) and whose variant is not `primary` or `danger-solid`.

The check reads `variant` statically. So a dynamic variant, like Inventory's `primary ? …`, must split into two JSX branches. The violations that exist today go on the shrinking offender list under their lane. Rule (c) is born in this branch, so it may add keys.

**What IconButton needs, beyond the brief:**
...
- A visual size of 28 in dense rows, with a hit area of 40 or more.
- The tooltip on hover, focus and long press.
- These kit controls also move onto it: overlay Close, the Toaster dismiss, the FacetChip clear, the SearchField clear, ReloadButton, the compact FilterBar trigger, and the SortControl direction.

## 3. Icons to add, and collisions

**Add 8 paths, each with an `ICON_MEANINGS` row:**
- `sold`: a round seal with $, for Mark sold and the Sold pill. It is not `tag` (Pricing) and not bare `dollar` (Sales).
- `pencil`: Edit or Rename.
- `eraser`: clear typed values, undoable.
- `eyeOff`: Hide.
- `sortAsc` and `sortDesc`.
- `moveTo`: an arrow into a bracket. Do not draw a box: `box`, `package` and `archive` are already box shapes.
- `grip`: six dots.

Also add `ICON_MEANINGS` rows for the icons that exist: `x`, `check`, `eye`, `lock`, `external`, `download`, `settings` and `hand`.

**Collisions to fix, by owner lane:**
- `x` as a write:
...
- `undo` where the press is not an undo:
  ...
  - Orders' not-shipping button (label, paraphrased) → no glyph.
...
- `lock` as Hide: Codes, 3 places → eyeOff. `lock` now means seal or hold only.
- `tag` and `wand` as manual edits: BoxOps Rename, Name sections and Set claims, and CardOps Correct claims → pencil. `wand` stays for "let Banchi check again".
- `package` as Move to box (BoxOps) → moveTo.
- `download` as a TCGplayer fetch: RunPanel, Markdown and LiveReconcile → refresh. `download` means a file to disk.
...
- `external` as the departed badge (BoxBrowse) and `eye` as "Viewing" → the inventory lane fixes these (UX-222, UX-221).
- `pin` as Keep open (PriceHistory) → words. `pin` means a pick location.
- `refresh` as a reset: Capture "Clear the setup" → eraser.
- **Retire against Delete:** there is no collision today, because `archive` is not `trash`. Keep them apart.

## 4. Risks

1. **Orders Mark sold as an icon, with a broken Undo.** Undo on Orders refuses today (UX-195, HOR-07), and the fix is in the later undo session. An icon press in a dense 8-row walk, with no working way back, can sell the wrong card. My recommendation: convert Orders' PickLine and RowAction only after the undo session, or have the owner accept the risk.
2. **D118 slot sizes.** Inventory reserves 137x28 for the Mark sold and Retire pair. Two 40x40 targets make each row 12px taller. That fights UX-206 and UX-169, which want a taller walk. The kit must draw the button at 28px and extend the hit area to 40px, and settle what happens where two hit areas overlap.
3. **No tooltips on touch.** On a phone, sold, archive, moveTo and eraser have no visible name. Apply the phone edge case in section 2.
4. **Findings this conflicts with:**
   - **UX-214 (FLT-19).** The kit's SortControl promises the order "never hidden in an icon". The sort key stays in words. The direction becomes a glyph, and its words move to the tooltip and the name. The owner should confirm this.
   - **UX-108 (shipping).** It asks for "a verb and its object" and names "Forget". As an icon, "Forget this file" moves to the tooltip only.
   - **UX-145.** The keycap stays after the label, but now the label is the tooltip. So R and U leave the face. The keys sheet still lists them.
   - **UX-099.** Danger tone and ask-first must follow kit-frame's one rule for destructive presses. An icon adds no second pattern.
   - These findings agree with the plan: UX-056, UX-267, UX-118, UX-221, UX-222 and UX-232 (it stays words).
   - Do not claim UX-195, UX-249, UX-252 or UX-204. The icon does not fix those. They belong to the undo session.
5. **Screen readers.** A walk with 30 buttons that are all named "Mark sold" repeats names (UX-271). Every row IconButton needs the object in its name.
6. **Tests.** Specs that find a button by its visible text (`hasText`, `text=`, the demo-coverage spec) will break. Find it by its role and name.
7. **Discoverability of Manage.** Manage becomes an icon, and the Orders fetch lives inside that sheet today (UX-193). The orders lane must move the fetch out first (HOR-05).
8. **Box-shaped glyphs.** `box`, `package` and `archive` already look alike, and a box-shaped `moveTo` would be a fourth. At 14px they blur. Draw `moveTo` with no box.

### Critical files for implementation
- /Users/shivinate/Developer/pkmnscan/.claude/worktrees/jovial-banach-362f31/app/src/kit/Icon.tsx
- /Users/shivinate/Developer/pkmnscan/.claude/worktrees/jovial-banach-362f31/app/src/kit/index.tsx
- /Users/shivinate/Developer/pkmnscan/.claude/worktrees/jovial-banach-362f31/scripts/kit-adoption.mjs
- /Users/shivinate/Developer/pkmnscan/.claude/worktrees/jovial-banach-362f31/app/src/Inventory.tsx
- /Users/shivinate/Developer/pkmnscan/.claude/worktrees/jovial-banach-362f31/app/src/Orders.tsx