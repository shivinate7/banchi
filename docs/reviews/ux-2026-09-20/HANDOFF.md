...
**The fixes are merged.** PR #451 landed on `main` as `bfbb5c2dc` with every check green.
What follows is what was deliberately NOT done, held at the owner's word.

The session that produced this work was stopped by a monthly spend limit and resumed on
credits. Nothing was lost.
...
1. **Integrated** — all five lanes merged into one branch with zero conflicts, each lane's
   commits kept. Merged as `bfbb5c2dc`.
2. **The cross-lane contrast pair verified on the merged result**, not assumed.
   `.graveyard-condition` measures 4.85:1 on the page and 4.55:1 on wells in light, 5.62:1
   and 5.73:1 in dark. One lane moved the call site onto `--bn-ink-3` while another darkened
   that token; neither could see the other, and it only resolved once both shared a branch.
3. **Three copy ceilings re-pinned** on the owner's word, measured rather than guessed:
   `#/orders` 127 to 129, `#/revenue` 103 to 106, and `#/review` 84 DOWN to 83 because the
   reason code came off the screen entirely. Net +4 words. That headroom is spent
   permanently — the ratchet only moves down.
