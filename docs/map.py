    ...
    },
    {
        ...
        "modules": {
            ...
            "src/kit/lockupGeometry.ts": {"does": "the lockup as static path data — 番地 and BANCHI OUTLINED, the bracket's arm and its two end discs, the block's dimensions and section 13's eleven settled parameters. GENERATED by scripts/build-lockup.mjs and never hand-edited. Every number is a ratio of the kanji size and the paths are drawn in a 319 x 233 box at kanji 100, so ONE geometry serves every size through a viewBox — which is not only smaller than per-size data but more correct, since a vector scaled by a viewBox cannot re-layout and live text could, and did: the width match had to be SOLVED per size. Carries no `fill`: the component's own <g> supplies it by inheritance so a stylesheet can switch section 16's dark metal, which a fill attribute on the child would make unreachable.",
                                 "governed_by": ["D102"]},
            "src/kit/Lockup.tsx": {"does": "the lockup — 番地 over BANCHI inside the mark's own brackets, drawn entirely from lockupGeometry.ts. THE ACCESSIBILITY CONTRACT INVERTS `Logo`'s AND THEN INVERTS AGAIN: `Logo` is aria-hidden always because every call site names its wrapper, and the lockup IS the word so standing alone it carries role=img and a name — but inside the sidebar's brand control — a <button> that collapses the rail, which already carries its own label — a named child would announce the name twice over it, so that call site passes `decorative`. Three class hooks (.bn-lockup-bracket, -kanji, -roman) exist so the shell can choreograph the collapse; without them it could only fade as one lump. The dark bracket is section 16's `bluesteel` chrome and is switched in CSS, not here — the gradient's id is per-instance, which a stylesheet cannot name, so the instance publishes it as --bn-lockup-metal and App.css decides per theme.",
                                 "governed_by": ["D102"]},
            "src/kit/RailMark.tsx": {"does": "the collapsed sidebar's mark: the small cut's two brackets with the card and the tile taken out. Section 1 names this rather than describing it — \"with the card removed the same brackets become an empty slot, which is the in-product mark\" — and the rail is where that reading is literal, the lockup being the name inside its address. NOTHING IS DRAWN HERE: the path and the weight are markGeometry.ts's SMALL_BRACKET and SMALL_STROKE, so this is `Logo`'s own wire with two elements omitted rather than a second drawing of it. HAVING NO GROUND IS WHY IT CANNOT TAKE THE CHROME FLAT: bluesteel's four bracket stops run #FFFFFF to #8FA4B8, bright on `Logo`'s dark tile and invisible on a light sidebar, so it takes section 16's own switch — flat --bn-ink on light, the chrome on dark — published as --bn-lockup-metal and decided in kit.css, on `stroke` rather than `fill` because this cut is a wire. `Logo` shipped in this slot until 2026-09-06 and read as the favicon rather than as the collapsed lockup.",
                                    "governed_by": ["D102"]},
