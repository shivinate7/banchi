...
COMPONENTS = [
    ...
    },
    {
        ...
        "modules": {
            ...
            "imb.py": {"does": "the Intelligent Mail barcode encoder — USPS-B-3200 Rev H, "
                               "steps 1 to 6, four data fields to the 65-bar string a "
                               "barcode font prints. A PURE FUNCTION over digits: no "
                               "network, no account, no credential, no per-piece fee, and "
                               "nothing in it can spend. That is the point of the module — "
                               "the IMb is arithmetic, and the services that sell PWE "
                               "tracking sell the platform around it, not the barcode.",
                       # D61 owns the envelope lane this feeds; D19's rule that a constant
                       # is derived rather than picked is why the two character tables are
                       # GENERATED by the spec's own routine and only the 65-row bar map is
                       # transcribed.
                       "governed_by": ["D19", "D61"],
                       "tested_by": ["T10"],
                       "note": "IT IS A LIBRARY AND NOT A FEATURE — no route, no client "
                               "function and no screen reaches it, which by CLAUDE.md's own "
                               "rule means it is not landed and must not be reported as "
                               "such. AND A BARCODE IS NOT TRACKING: scan events exist only "
                               "where the Mailer ID it carries is registered to an Informed "
                               "Visibility subscription AND the piece runs on mail "
                               "processing equipment. Both are account facts this repo does "
                               "not hold and this module deliberately does not model. THE "
                               "MAILER ID HAS NO DEFAULT, on purpose: a made-up MID is "
                               "either somebody else's scan data or nobody's, and the second "
                               "failure is an envelope that is simply never reported. THE "
                               "SERIAL IS STATE THIS MODULE DOES NOT HOLD — it must be "
                               "unique per piece within a MID or two envelopes report as "
                               "one, which is a wrong answer shaped exactly like a right "
                               "one, and allocating it belongs beside the order ledger."},
