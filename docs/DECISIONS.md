...
**What went wrong is that a box passes 999 cards.** The owner's largest holds 723 and he says a thousand is imminent. Measured at 1440 against the shipped column: a three-digit row leaves 18.8px between the figure's ink and the path, **a four-digit row leaves 5.6px, and a five-digit row OVERLAPS the path by 7.6px.** None of it is visible to an element-box measurement — `.position-num` is `flex: 0 1 auto` in a fixed-width slot, so it is shrunk to an identical width at three, four and five digits and its glyphs paint outside it with `overflow: visible`. The first draft of the test asserted exactly that, off `getBoundingClientRect()`, and **was green against the defect it was written for**; `app/tests/inventory.spec.ts` reads `scrollWidth` and a `Range` over the text node instead, and says why in the case.

**A second instance of the same class was already live and nobody had hit it.** `--pos-slot-col` was in `rem` while every quantity it contained was in `px` — `--pos-slot: 22px`, and `.position-key`'s clamp sitting on its 11px floor — and `app/src/base.css` sets no root font size, so a reader on a 12px root font overflowed the column **at three digits, today**. `--pos-slot-key` is `px`.

**And the two-fonts'-`ch` error this entry was written about turned out to have one more axis.** `.position-void` declared no `font-family`, so it inherited Inter and reserved 41.63px against the figure's 44.47px of Manrope — the original bug, still in the file, hidden only by the hard width. Giving it the family left it **3.156px short**, because `1ch` is the advance of `0` in the element's own FACE and Manrope is a variable font: 14.828px at weight 800 against 13.772px at 400. The rule the third instance earns: **a `ch` reservation that has to match another element's must match the whole face, family and weight both** — so the void carries the figure's face for its metrics and paints its em-dash at the muted weight through its own `::before`. It is `box-sizing: content-box` for the same reason, because `border-box` made the key's padding subtract from the digits' reservation instead of adding to it (measured: 27.3px short, thirteen times the defect this column was built to fix).
