...
**`:active` IS STILL NOT DONE AND THE OWNER HAS QUESTIONED THE DEFERRAL.** Asked whether to reopen it they replied *"why would we decline?"* — recorded here as an open question rather than settled either way. What this entry gave as the reason is now weaker than when it was written: "only three files have one today" is **16 files and 29 rules**, grown with no floor and no guard, and the "reflow risk" is answerable, since the kit already presses with `transform: translateY(1px)`, which is composited and moves no neighbour. The 22 controls with no hover response at all are likewise unresolved and mostly legitimate — 9 are the current page's own nav link, and the rest are active tabs and segments, which have nowhere to move to. **Neither is built here, and neither should be read as declined.**
