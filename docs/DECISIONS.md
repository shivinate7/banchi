...
**AMENDED 2026-09-06 — THE RESPONSE FLOOR, WHICH IS THIS ENTRY'S OWN TITLE FINALLY REACHING `base.css`.** The owner reported that hovering "isn't giving an intuitive feel" and, asked which control, named the sidebar's rail toggle: *"the borders of for example the lockout icon to open close the sidebar feel wrong."* That control is `.bn-brand`. It declares `border: 0` and `:hover { background: var(--bn-surface-3) }` with **no `transition` at all**, so its 109px hover slab arrived in one frame while the `.bn-nav-link`s directly beneath it in the same sidebar faded a much smaller pill over 120ms. What reads as a border being wrong is a hard edge appearing with no ease under a pointer that is still moving.
