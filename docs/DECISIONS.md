...
### It wanted a route on D49's precedent, and the nav has no room for one
...
**The strip needs 1,484.9px to draw eleven links on one row and the owner's desk is 1,440.** The aside group wraps, the nav doubles from 45.5px to 90px, and `app/tests/review.spec.ts`'s between-cards floor — D28's, the one that says the photograph does not move as the operator answers — goes red as the web font swaps in and tips the wrap mid-screen. **No label rescues it**: the eleventh link would have to be 68px wide and `Runs` is 72.6.

**So the choice was this screen's route or the shell's layout, and the shell is being rewritten on another branch.** The screen is a section on `#/runs` beneath the store-wide reconcile, which is the right neighbour regardless: both read the same file, and the order there is the order of the work — settle what TCGplayer holds, then decide about the part of it that is not moving.

**What this costs is a hash of its own**, so the markdown cannot be linked to, bookmarked, or reached by a chord, and it sits below a screen whose first two thirds are about one run. `App.tsx`'s ROUTES table carries the measurement where the row would have gone, so the next session to want an eleventh route finds the number rather than rediscovering it.
