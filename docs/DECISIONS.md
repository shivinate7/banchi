...
- **A realised-price series already exists, in an OrderWand export the owner holds** (recorded 2026-08-30). `orderwand_2026-08-30.csv` — 715 sale lines across 543 orders, 2026-05-05 to 2026-08-30, $56,194 of realised sales — exported from the OrderWand browser extension, a buyer-side order-history tool that also exports seller sales. Every row carries a price **and** a `Vendor Product Id`, and that id is a TCGplayer `productId`, which is the key `infinite-api` already takes. So the file joins to the series in the entry above with no lookup at all, and answers the one question that series cannot: not what the market did, but what this seller actually got for it.
