...
**`_order_progress` answers `pulled: [{capture_id, box, index}]`, composed per read from `card_by_capture_id`, and nothing writes it down.** The copies panel needs it and can get it no other way: a pulled copy is SOLD, so the resolver offers it in no pick, and `SearchCopy` carries no capture id for the ledger's records to meet it on. The ledger holds capture ids, the walk is keyed by position, and this is the join between them.
