...
**STATUS, 2026-09-03: SPECIFIED and BUILT. NOT VALIDATED.** The two commands, the four routes,
...
upload is a manual step. Reachable on `#/runs`, beneath the store-wide reconcile — see §5
for why it is a section there and not a route of its own.
...
### It is a section on `#/runs`, and that is a measurement

D49's precedent says a worklist the operator sits in earns a route, and this is one. `#/markdown`
was built and then measured out: **the nav strip needs 1,484.9px to draw eleven links on one row
against the owner's 1,440.** The aside group wraps, the nav doubles from 45.5px to 90px, and
`app/tests/review.spec.ts`'s between-cards floor goes red as the web font swaps in and tips the
wrap mid-screen. No label rescues it — an eleventh link would have to be 68px and `Runs` is 72.6.

So it sits beneath the store-wide reconcile, which reads the same file. **What that costs is a
hash of its own**: no link, no bookmark, no chord, and it is below a screen whose first two thirds
are about one run. `app/src/App.tsx`'s ROUTES table carries the measurement where the row would
have gone.
...
| screen | `app/src/Markdown.tsx`, a section on `#/runs` |
...
| screen tests | `app/tests/markdown.spec.ts` |
...
- **A nav that fits eleven links.** The route this wanted is a change to the strip's design,
  not to this screen, and the number to beat is in §5.
