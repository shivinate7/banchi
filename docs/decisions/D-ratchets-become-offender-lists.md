...
- **A reflow.** Whitespace is collapsed before the hash. A code span can cross a line break, and the vendored linter masks code one line at a time. So before the hash, the lines that one code span runs across are joined into one line (`join_code_span_breaks` in `scripts/ste_measure.py`). The linter then hides the code span over the joined paragraph. The same sentence then gives the same findings and the same hash, wherever its line breaks fall.
...
- Only code spans are joined across a line break. A link, an HTML tag or a URL that crosses a line break is still masked one line at a time. So a reflow into or out of such a construct can still change a sentence's hash.
