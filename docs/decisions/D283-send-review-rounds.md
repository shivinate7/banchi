...
- **An unclear answer.** A publish that TCGplayer answered 500, a publish that never answered,
  and a rollback that TCGplayer refused are UNKNOWN. The copies stay counted, and the claim holds
  their SKUs out of every send. The card names the upload that may wait in TCGplayer's
...
- **The same bytes.** A second push of the same file bytes is refused only inside the upload
  window. There, the live read cannot see the first push yet.
...
- **A mark-down's new price. REMOVED IN ROUND 4: the premise was false.** Round 3 made a
  listing send leave out the cards that a mark-down re-priced inside the publish lag. The aim
  was that a listing row could not put the old price back. But `reprice apply --write` writes the new
...
- **H1, one rise, two credits.** The check gave each SKU's live rise to the OLDEST due
  ...
  first to sends that Banchi saw go live, then to sends it did not see confirmed. Downloaded
  files come last, because their upload time is not known. A send still inside its wait is
...
- **B1 and B2, a price the button did not name.** The screen compared a typed price with the
  join's old export, and the server compared it with a fresh one. The server also counted every
  corpus answer as typed. So a Live tab preset, or a mark-down written and never sent, could go live on the next
  send. Now only a SKU the owner typed on the worklist in this visit rides. The screen sends it
  with the price and the live price it drew. The server refuses a named price whose live price
