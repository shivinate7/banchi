...
**THIS ENTRY WAS RE-SCOPED THE DAY IT LANDED AND THE RE-SCOPE IS ITS OWN ENTRY.** Everything above stands — the name, the four shapes, the frozen-not-tracked distinction, the four rejected alternatives and every measurement. What the owner rejected was stopping here: giving the card a name and leaving the PHOTOGRAPH filed under the address, so that *"the move is still a tombstone plus hand-moved copies and the renumber is still 1,608 filesystem operations"* two paragraphs up stayed true. **The photograph is stored under this name now**, which retires that work instead of pricing it. See `D183` and `docs/specs/stable-card-id.md` §0; the two paragraphs of this entry that a reader would otherwise take as the final scope are the one beginning *"The name is additive and the reverse is byte-exact"* — the operation is no longer confined to the database — and the one above.
