...
- **"Check first"** opens the composer on its default start, every card photographed and not identified. That is the same N the strip names. Its flow did not change: the free pre-check, then the confirm, then the spend, over D180's selection grammar. A composer opened this way and closed with no run started closes the sheet too, so the operator is back on Review.
- **"Identify now"** starts the paid run at once. It sends the composer's own default send (`RunsComposer.tsx:defaultSend`, one derivation beside the composer's), with no pre-check and no confirm screen. The receipt is a toast, then the run open in the Runs sheet, where its progress reads itself, the way a composer-started run's does.
- **The server did not change.** The one spend route asks only for `confirm: true` (D33). It holds no pre-check token, so a press that skips the pre-check needs nothing new. D174's claim is still written by the command, inside the transaction that decides which cards it buys. A second tab can never buy the same cards twice. `make submission-selftest` proves that half. The screen adds an in-flight guard, so a double press sends one spend. `review.spec.ts` proves that half, and it goes red with the guard deleted.
