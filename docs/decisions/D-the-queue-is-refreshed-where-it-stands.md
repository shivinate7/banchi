...
**Over one run's own export, the refresh and a real join reach the same verdict on every position both see: 172 of 172 the same queued-or-listed, and 81 of 81 the same reason AND the same candidate rows.** Zero disagreements. That number was 177 of 183 and then 172 of 172 across two real defects this measurement found and nothing else would have:
...
**What it repairs, measured against the 513 the finding names**, on a reconstruction of the store as it stood (the entries reopened and their answers withdrawn on a throwaway copy, because a human has since answered 564 of the 565 by hand): **437 repaired — 340 resolve outright and leave the queue, 97 refreshed in place, 196 candidate rows gain a rarity, 880 off-condition rows dropped, and 4 become answerable in one tap.** The remaining 76 are named rather than counted: 68 cards that have left the box, 7 with no stored reading, 1 already current.

**Those figures are D162's as much as this command's, and the split is worth recording**: before that entry merged the same pass over the same store resolved 199 and refreshed 238. The work the operator would have done by hand on 141 of those cards is what one merge plus one press now does, and neither half could have done it alone — D162 could not reach a frozen entry, and a refresh against the older ladder would have left every one of them queued.
