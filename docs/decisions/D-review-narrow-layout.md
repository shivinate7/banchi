...
**Not built this round.** UX-205 is a different defect: a receipt that never renders, not one rendered off-screen. `remember` runs only when `canTakeBack` is true, so six of nine answers in one session drew no receipt at all. It needs a wider look at `remember`'s own callers before a fix lands. UX-255 (five names for one closing act: Close / Close without answering / Stand down / Stood down / closed) is a design question the round's rulings do not answer. See PROGRESS.md.
