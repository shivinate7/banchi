...
**A screen is read only once it is loaded.** `routeSweep.ts:openSettled` waits for five things. `.bn-shell[data-route]` names the route. Exactly one `.bn-view` exists. Nothing is `[aria-busy="true"]`. No `fetch` or `xhr` read is still open. `.bn-view`'s text holds still for 450ms. A screen that never settles fails the run and names the route. The open-read count is necessary, because `#/inventory`'s "Reading the inventory…" has no `aria-busy`. It was measured on 2026-09-24, with every inventory read held for 900ms. A text-stability wait alone read that route at 4 words. The full wait read the painted 197.
