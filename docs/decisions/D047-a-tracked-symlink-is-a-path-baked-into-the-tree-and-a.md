...
### Amended 2026-09-12: the retry loop rescues a poisoned batch and nothing rescues a poisoned candidate

**`ignored_paths`'s single-candidate retry — the fix two sections above — has a hole its own docstring did not name: it assumes asking a candidate ALONE always gets a real answer, and a symlink boundary means it never does.** Found the same way the original defect was: a commit blocked over a path that was not wrong. This time the candidate was `harness/images/images`, named in `docs/specs/mechanization-backlog.md`'s own account of the ORIGINAL incident above — describing, in prose, the exact self-referential link this entry is about — and it failed in a **worktree**, where `harness/images` is a real symlink to the main checkout (`scripts/worktree-guard.sh`'s own provisioning, doing exactly what it is supposed to do).

**`git check-ignore` refuses ANY pathspec that walks past a symlink — one candidate, asked alone, still exits 128** with `fatal: pathspec 'harness/images/images' is beyond a symbolic link`. The retry loop's own contract — "ask again one candidate at a time so a refusal is contained to the candidate that caused it" — assumed containment meant recovery. It does not, when the SAME failure the batch hit recurs on that one candidate in isolation, because the cause is not which candidates share a batch; it is that the FIRST SEGMENT of this specific candidate is a symlink, and no batching strategy changes what git's pathspec matcher will do with a path that walks through one.

**The fix reads an ancestor instead of the candidate.** Gitignore's own directory semantics make this exact rather than a guess: a pattern that matches a directory ignores everything beneath it, so if `harness/images` — the symlink node itself, asked with NO trailing slash, the one form `check-ignore` can still answer past its own boundary — is ignored, so is every path under it, symlinked or not. `ignored_paths` now walks up from a candidate that failed alone, asking each ancestor in turn, and stops at the first one `check-ignore` can actually resolve.

**`ignored_paths` gained an optional `root` parameter for exactly one reason: so the self-test could reproduce this without touching the real checkout's own `.gitignore` or symlinks.** A throwaway git repo, a real symlink, both directions proven — a symlinked, ignored ancestor is recognised, and a symlinked ancestor that is NOT ignored is not swept in with it by the same walk. Every caller in this file still omits it.

**What this changes about the paragraph three sections up**, "So a batch that did not run cleanly is not evidence about anything": that sentence is still true of the BATCH. It was never true of a SINGLE candidate that fails alone, and this entry did not say so until now.
