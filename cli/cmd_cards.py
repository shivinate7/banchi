...
FIVE SUBCOMMANDS AND THREE OF THEM WRITE NOTHING EVER.
  ...
  cards checks    the four stored-data identification checks (D-archive-store-checks): a
                  name too long, a denominator or digit count that disagrees with its set,
                  a name one edit from a sibling in the same set. Review signals, never
                  repairs, never a catalogue call.
...
`name`, `audit` AND `checks` OPEN THE STORE READ-ONLY AND MUST NEVER CALL `db.connect`. That
function is the single entry to the store and it always calls `_ensure_schema`, so a preview
routed through it would PERFORM the migration it claims to be previewing. It is a hard
property with a harness arm behind it, and it is why this module talks to `sqlite3` directly
instead of going through `store.session`.
...
def _checks(args, say) -> int:
    """The four stored-data checks `D-archive-store-checks` builds: read-only, no catalogue,
...
def run(args, say) -> int:
    ...
    if handler is None:
        say("pkmnscan cards <name|audit|checks|photos|variants>")
        return 2
