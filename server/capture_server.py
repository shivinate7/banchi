    ...
    with store.write() as snapshot:
        ...
        if pair is None:
            raise BadRequest(
                HTTPStatus.CONFLICT,
                "not_corrected",
                f"{join.said_place(snapshot.inventory, box, index)}'s correction record carries "
                f"nothing to put back, or is malformed, so there is nothing here to guess. "
                f"Correct the card by hand.",
            )
    ...
    with Store().write() as snapshot:
        ...
        if not card.sku:
            raise BadRequest(
                HTTPStatus.CONFLICT,
                "not_identified",
                f"{join.said_place(snapshot.inventory, box, index)} carries no SKU yet, so there is no listing here "
        ...
        if getattr(card, "identity_source", None) == master.IDENTITY_SKU:
            raise BadRequest(
                HTTPStatus.CONFLICT,
                "already_confirmed",
                f"{join.said_place(snapshot.inventory, box, index)} already draws its listing's own name — there is "
    ...
    with store.write() as snapshot:
        ...
        if getattr(card, "bound_by", None) != "confirm":
            raise BadRequest(
                HTTPStatus.CONFLICT,
                "not_confirmed",
                f"{join.said_place(snapshot.inventory, box, index)} carries no standing confirm this route wrote, so "
        ...
        if restores_to is None:
            raise BadRequest(
                HTTPStatus.CONFLICT,
                "not_confirmed",
                f"{join.said_place(snapshot.inventory, box, index)}'s confirm record carries nothing to put back, so "
...
def _card_facets(
    ...
    for _, (game, set_name, rarity, state) in inventory.cards.select(
        ...
        # S3, THE OPUS REVIEW, 2026-09-25: D132 hides every DEPARTED card behind Hide sold,
        # never SOLD alone. `_card_facets` counted a retired or moved card into a bucket
        # the walk it labels never shows, the same disagreement UX-210 measured for a sold
        # one — `master.TERMINAL_STATES` is the one tuple `_box_row`'s own `matches` count
        # and D132's toggle both already answer to.
        if hide_sold and state in master.TERMINAL_STATES:
            continue
