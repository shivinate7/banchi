...
make map            # docs/map.py RENDERED (D80). ARGS=<package|path|D<n>|--stale>.
                    ...
                    #   default view now also names the entry's path, because the
                    #   filenames are zero-padded (D42 is `D042-...md`) and a glob
                    #   typed from a citation matches nothing and says nothing.
