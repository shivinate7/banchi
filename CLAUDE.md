...
make design-check   # DESIGN.md's Fulfillment floors, in a browser. TAKES A MACHINE-WIDE LOCK
                    ...
                    #   `make check` (docs/DEBTS.md §16).
                    #   `PW_ARGS=<flags>` reaches Playwright. `ARGS` never does (D136). CI runs
                    #   3 shards of 1 worker each. NEVER RAISE THE WORKER COUNT (DEBTS §8).
                    ...
                    #   documented traps in docs/DEBTS.md). Guarded twice: `make docs-audit`'s
...
- **The capture server is ALREADY THREADED and NOT YOURS TO RESTART.**
  ...
  sends `Connection: close` (docs/DEBTS.md §11 has the mechanism and the measurements — this
...
- **FIX THE CAUSE, NEVER THE SYMPTOM. CHECK WHETHER THE PRIMITIVE ALREADY EXISTS FIRST.**
  ...
  still be the right call. Call it a stopgap fix in the commit, with the cause in docs/DEBTS.md.
...
- `docs/DEBTS.md` — known gaps in the verification tooling, deliberately unfixed, never
  blocking. Read it before treating green `docs-audit` as coverage.
