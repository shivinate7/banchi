...
test.describe('the set view', () => {
  ...
  test('a tap lands on the ordinary box walk, at that card, with Sets left behind', async ({ page }) => {
    ...
    // THE TAB SWITCH FOLLOWED THE URL, because it reads the same `view` param.
    await expect(page.getByRole('tab', { name: /Shelf/ })).toHaveAttribute('aria-selected', 'true')
    await expect(page.getByRole('tab', { name: /Sets/ })).toHaveAttribute('aria-selected', 'false')
