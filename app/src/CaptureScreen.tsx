...
type BoxOption = {
   ...
   *  parts and not the total. The hand order's second term (D141). `null`
 ...
 *
 * THAT ENTRY PUT SEVEN HERE AND THE OWNER HAS SINCE OVERRULED SIX (D141):
...
export function CaptureScreen() {
  ...
  /* THE SETUP THIS BROWSER HAD LAST TIME, read once and then owned by the six `useState`s
     below (D141). One `localStorage` document rather than six keys, because
  ...
  /* WHICH BOXES THIS BROWSER HAS REACHED FOR, newest first, shared with `#/inventory`'s rail
     (D141). Read once into state and advanced by `touchBox`'s return value
  ...
  /* THE SETUP, WRITTEN WHOLE ON EVERY CHANGE TO ANY OF IT (D141). One
  ...
  /* THE BOX LIST, IN THE ORDER THE OPERATOR'S HAND WORKS (D141).
   *
   ...
   * (D141). This took the LAST nine with nothing typed until then — the
   ...
   *
   *  THE REACH IS RECORDED HERE AND NOWHERE ELSE ON THIS SCREEN (D141).
  ...
  /** ONE PRESS PUTS THE SETUP BACK TO NOTHING CHOSEN (D141).
   *
   ...
   *  it (D141). A control that appears and disappears with the state it acts
  ...
  /* The stage foot's word for the drawer: the name, or the number where there is no name
     (D141). This read `Box 3 · RB Epics` and carried the number twice on one
    ...
    <main className="capture bn-page" data-mode={triggerMode} data-live={camera.ready ? 'true' : 'false'}>
      ...
      <div className="capture-shell">
        ...
        <section className="capture-card capture-card-run" aria-label="Run">
                ...
                {boxRows.map((option) => (
                  <Opt
                    ...
                    /* THE NAME LEADS AND THE NUMBER FOLLOWS IT, which is the reverse of what
                       this drew until 2026-09-11 (D141). The number STAYS
            ...
            <Row
              ...
              right={
                box === null ? (
                  ...
                  /* THE NAME IS THE HEADLINE AND THE NUMBER IS GONE (D141).
          ...
          {box !== null && !boxIsEmpty ? (
            <div className="capture-onward">
              ...
              >
                {/* THE NAME HERE TOO (D141), even though the destination
        ...
        <section className="capture-card capture-card-rig" aria-label="Rig">
          ...
          {/* THE CLEAR, AT THE FOOT OF THE LAST PANEL IN THE RAIL (D141).
